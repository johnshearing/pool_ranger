...
spending/private key) with a cooperative administrator.   
The administrator delegates the
combined stake to carefully chosen pools. 
Members can revoke at any time.  
99% of rewards flow back
...
- Address: stored in `_1_members.json` under the `admin_0` row (the `address` field). There
  is no `0_admin_0.addr` file on disk anymore — every script reads the admin's bech32 address
  from `_1_members.json`.
- There is **no `0_admin_0.sk`** — the private key never leaves the device.
- Any script that needs to act as admin must use a Ledger signing flow (CIP-30 in-browser or
  HID in CLI), not a software key.
- Scripts that only read admin balance look up the address in `_1_members.json` and pass it
  to Blockfrost directly.
