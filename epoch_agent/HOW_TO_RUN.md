...
1. The Agent, using the Epoch Agent, creates a candidate list that contains every Cardano stake pool that has been running for at least 30 epochs. Pool Ranger intentionally considers newer pools (those in the 30–73 epoch range) as candidates. Supporting newer, smaller pools promotes Cardano decentralization — one of Pool Ranger's core goals alongside maximizing member ROA.  
